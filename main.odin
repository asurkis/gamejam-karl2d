...
SUN_RADIUS :: 1
STARTING_RADIUS :: 3
GRAVITY_STRENGTH :: STARTING_RADIUS * STARTING_RADIUS * STARTING_RADIUS
...
init :: proc() {
	k2.init(1280, 720, "Web example", {.Windowed_Resizable})
	init_game_state()
}
...
init_game_state :: proc() {
	...
	for i in 0 ..< MAX_SHIPS {
		...
		ship.vel = STARTING_RADIUS * {-sin, cos}
...
step :: proc() -> bool {
	...
	for i in 0 ..< MAX_SHIPS {
		ship := &ships[i]
		distance_from_sun := linalg.length(ship.pos)
		acc :=
			GRAVITY_STRENGTH *
			-ship.pos /
			distance_from_sun /
