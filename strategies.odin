...
strategy_player :: proc(ship: ^Entity, ship_id: int) {
	...
	if k2.key_is_held(.Up) do ship.engine_control.y += FORWARD_ENGINE_STRENGTH
	if k2.key_is_held(.Down) do ship.engine_control.y -= BACK_ENGINE_STRENGTH
	if k2.key_is_held(.Right) do ship.engine_control.x += SIDE_ENGINE_STRENGTH
	if k2.key_is_held(.Left) do ship.engine_control.x -= SIDE_ENGINE_STRENGTH
...
strategy_cpu2 :: proc(ship: ^Entity, ship_id: int) {
	...
	engine_control_variants[1].y = FORWARD_ENGINE_STRENGTH
	engine_control_variants[2].y = -BACK_ENGINE_STRENGTH
	engine_control_variants[3].x = SIDE_ENGINE_STRENGTH
	engine_control_variants[4].y = -SIDE_ENGINE_STRENGTH
	...
	for variant_engine_control in engine_control_variants {
		...
		if variant_min_distance_to_sun > SHIP_RADIUS + SUN_RADIUS {
	...
	if frame_distance_of_shot <= 3 * SHIP_RADIUS do ship_shoot_bullet(ship)
}

strategy_cpu3 :: proc(ship: ^Entity, ship_id: int) {
	ship_shoot_bullet(ship)
}
